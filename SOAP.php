...
function threedeeuserthumbnail($userId) {
    ...
    if ($RCCServiceSoap instanceof SoapFault) {
        killExeThumb($randomport);
        return json_encode(['Success' => false, 'Message' => 'Service Fault: ' . $RCCServiceSoap->getMessage()]);
    }
    ...
    // Prepare the script
    $scriptText = 'local plr = game.Players:CreateLocalPlayer(0)
...
print(game:GetService("ThumbnailGenerator"):Click("PNG", 1920, 1920, true))}';
    ...
    if (isset($value->Error)) {
        return json_encode(['Success' => false, 'Message' => $value->Error]);
    }

    // Check if the expected value is returned
    if (empty($value) || !isset($value[0])) {
        return json_encode(['Success' => false, 'Message' => 'No data returned from the service.']);
    }

    // Return the thumbnail data
    $base64Image = $value[0];
    return json_encode(['Success' => true, 'Value' => $base64Image, 'ServicePort' => $randomport]);
}
