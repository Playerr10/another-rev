...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if($PlaceID)
	{
		$PlaceCheck = Assets::fetchCreation($PlaceID, $userAuth->id);

		if (!$PlaceCheck) die("You do not own this Place");
		if ($PlaceCheck->type != 0) die("Not a Place");
	}
	else
	{
		if (strlen($Name) == 0) die("Place Name cannot be empty");
		if (strlen($Name) > 50) die("Place Name cannot longer than 50 characters");
		if (strlen($Description) > 1000) die("Place Description cannot longer than 1000 characters");
	}

	$xml = file_get_contents('php://input');
	if (!stripos($xml, 'roblox')) $xml = gzdecode(file_get_contents('php://input'));
    $object = simplexml_load_string($xml);if ($object == false)die();
	
	if (strlen($xml) > 32000000) die("Place cannot be larger than 32 megabytes");

	if ($PlaceID) 
	{
		file_put_contents($assetPath . $PlaceID, $xml);
	}
	else
	{
	
        $PlaceID = Assets::availableId();
		$conn->prepare("INSERT INTO asset (`name`, `info`, `authorid`, `createdon`, `public`, `offsale`, `type`) VALUES (?,?,?,?,?,0,0)")->execute([$Name, $Description, $userAuth->id, time(), $Uncopylocked ? 1 : 0]);
		$conn->prepare("INSERT INTO inventory (`userbought`, `boughtid`, `boughtdate`, `type`, `wearing`) VALUES (?,?,?,0,0)")->execute([$userAuth->id, $PlaceID, time()]);		
		
	}

	file_put_contents($assetPath . $PlaceID, $xml);
	Users::updateCooldown($userAuth->id);
    AssetRender($PlaceID, 0);
}

$stmt = $conn->prepare("SELECT * FROM asset WHERE authorid = :sid AND type = 0");
$stmt->execute([':sid' => $userAuth->id]);
	...
	<body scroll="no">
		<script type="text/javascript">				
			...
			function publish(PlaceID)
			{
				...
				try 
				{
                    window.external.Write().Upload('http://<?=$_SERVER['HTTP_HOST']?>/IDE/Upload.aspx?PlaceID='+PlaceID+'&Name='+encodeURI(name)+'&Description='+encodeURI(desc)+'&PublicDomain='+free);
                    $("#Uploading").hide();
                    $("#Uploaded").show();
               	}
                catch(e) 
		...
		<div id="Default">
			<table height="100%" cellpadding="12" width="100%">
				...
				<tr valign="top">
					...
					<td>
						<div id="SaveText" style="display:block;"><strong>Create a new Place on ROBLOX.</strong> <br /> Choose this to create a brand new Place. Your existing Places will not be changed.</div>
					</td>
				</tr>
				<?php if ($Places) { ?>
				<tr valign="top">
					<td width="120"><input class="OKCancelButton" style="WIDTH: 100%"  type="button" value="Update" onclick="updatePlace();"/></td>
					<td><strong>Update an existing Place on ROBLOX.</strong> <br> Choose this to make changes to a Place you have previously created. You will have the opportunity to select which Place you wish to update.</td>
				</tr>
				<?php } ?>
				<tr valign="top">
		...
		<div id="NewPlace" style="display:none">
			<table height="100%" width="100%" style="padding-top:12px">
				<tr valign="top">
					...
					<td><input type="text" style="width:100%" id="Name" value="<?=$userAuth->username?>'s Place"></td>
				</tr>
				<tr valign="top">
		...
		<div id="ExistingPlace" style="margin-top:4%;display:none">
			...
			<span><a href="#" onclick="publish(<?=$Place->id?>)">Select</a> <?=htmlspecialchars($Place->name)?></span><br/>
			<?php } ?>
		</div>
