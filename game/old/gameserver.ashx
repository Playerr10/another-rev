...
// if(!empty($cookie)){ to do: add gameserver handling

	// $Parameters->Username = $userAuth->username;
	// $Parameters->UserID = $userAuth->id;
	// $Parameters->CharacterAppearance = getUserWearing(0, $userAuth->id, null, "client");

// }
$egghunt = (isset($_GET["egghunt"]) ? $_GET["egghunt"] : "false"); // sets server egghunt
?>

local placeId, sleeptime, port, url, timeout = <?=$Parameters->PlaceID;?>, 0, <?=$Parameters->Port;?>, "http://www.fleian.xyz", 0
